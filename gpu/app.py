"""
ImageProcessor (GPU build) — batch background-removal + compositing tool.

Same as the CPU build, but uses onnxruntime-gpu with CUDAExecutionProvider.
On first run the app probes for CUDA. If found, all rembg sessions run on
the GPU (~10x faster than CPU). If not, it falls back to CPU and prints a
warning so the user knows why it's slow.
...
def detect_providers():
    """Return (providers_list, label). Tries CUDA, falls back to CPU silently.

    Some machines list CUDAExecutionProvider but fail to actually load it
    (missing CUDA Toolkit, mismatched cuDNN). We accept the provider list
    optimistically and rely on rembg/onnxruntime to fall through to CPU at
    session-creation time if CUDA init fails — onnxruntime does this on its
    own when given multiple providers.
    """
    try:
        import onnxruntime as ort
        available = ort.get_available_providers()
    except Exception:
        return (["CPUExecutionProvider"], "CPU (onnxruntime missing)")

    if "CUDAExecutionProvider" in available:
        return (["CUDAExecutionProvider", "CPUExecutionProvider"], "GPU (CUDA)")

    return (["CPUExecutionProvider"], "CPU (CUDA not available)")


_PROVIDERS, _PROVIDER_LABEL = detect_providers()
...
def get_session(name):
    with session_lock:
        if name not in session_cache:
            try:
                session_cache[name] = rembg.new_session(name, providers=_PROVIDERS)
            except TypeError:
                # Older rembg versions don't accept the providers kwarg.
...
class App:
    ...
    def __init__(self, root, cfg, prefilled=None):
        ...
        self.root.title(f"Image Processor — {_PROVIDER_LABEL}")
        ...
        # Surface the active execution provider so the user knows whether
        # they're getting the GPU acceleration this build promises.
        self._log_main(f"Execution provider: {_PROVIDER_LABEL}")
        if "CUDA" not in _PROVIDER_LABEL:
            self._log_main(
                "WARNING: this is the GPU build but CUDA is not available. "
                "Install the NVIDIA driver + CUDA Toolkit, or use the CPU build."
            )
...
def run_cli(args, cfg):
    ...
    cli_log(f"Execution provider: {_PROVIDER_LABEL}")
    if "CUDA" not in _PROVIDER_LABEL:
        cli_log("WARNING: GPU build running on CPU — no NVIDIA acceleration available.")
